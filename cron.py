...
@app.function(schedule=modal.Period(hours=1), concurrency_limit=1, timeout=300, image=custom_image, secrets=[modal.Secret.from_name("azure-translation-key"), modal.Secret.from_name("database-url")])
def translate():
    import os
    import psycopg2
    import requests
    ...
    try:
        ...
        query = """
        (
            SELECT id, category, title, ingredients, instructions, language, "translateTo", "createdById", "modifiedAt", "createdAt"
            FROM recipe
        ...
        for (source_lang, target_langs), lang_recipes in recipes_by_language.items():
            for i in range(0, len(lang_recipes), 5):
                ...
                for recipe_idx, (id, _, _, _, _, _, target_langs, created_by_id, modified_at, created_at) in enumerate(batch):
                    ...
                    for target_lang in target_langs:
                        ...
                        insert_query = """
                        INSERT INTO recipe 
                        (category, title, ingredients, instructions, language, "translatedFromId", "createdById", "modifiedAt", "createdAt")
